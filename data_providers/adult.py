...
class Adult(data.Dataset):
    ...
    def __getitem__(self, index):
        return (self.x_n[index], self.x_c[index]), self.targets[index]


class AdultDataProvider:
    @staticmethod
    def name():
        return 'AdultDataProvider'
    ...
    def train_dataset(self, _transforms):
        dataset = Adult(root=self.save_path, split='train')
        return dataset

    def val_dataset(self, _transforms):
        dataset = Adult(root=self.save_path, split='val')
        return dataset

    def test_dataset(self, _transforms):
        dataset = Adult(root=self.save_path, split='test')
        return dataset
